...
ROADMAP_JOB_SIMULATOR = {
    '자동차 정비': {'고2_심화': ['EV 정비 기초 (전기/배터리)', '엔진/섀시 정밀 진단 실습', '특수용접 기능사 또는 피복아크용접 기능사 준비']},
    '자동차 차체수리': {'고2_심화': ['차체 구조 복원 심화 실습', '판금/용접 집중 훈련', '차체수리 기능사 및 용접 기능사 준비']},
    '자동차 도장': {'고2_심화': ['색상 조색 심화 교육', '스프레이 건/표면 처리 기술 집중 훈련']},
    '자동차 튜닝': {'고2_심화': ['랩핑/내장재 튜닝 실습', '기능 튜닝 (서스펜션/브레이크) 실습', 'CAD/3D 프린팅 기초']},
    '서비스 어드바이저 (SA)': {'고2_심화': ['고객 응대/상담 화법 훈련', '자동차 보험/법규 이해 및 진단 보고서 작성']},
    '자동차 딜러 (영업)': {'고2_심화': ['마케팅/세일즈 기초', '신차 프리젠테이션 훈련', '다양한 브랜드 스터디']}
}
...
def apply_custom_styles():
    """앱 전체의 커스텀 CSS (기본 폰트 사용, 글꼴 오류 방지)"""
    st.markdown(
        """
        <style>
        /* 폰트 깨짐 방지를 위해 커스텀 폰트 제거하고 시스템 기본 폰트 사용 */
...
def stage_3_roadmap():
    ...
    if selected_job:
        ...
        # DataFrame으로 학년/학기별 계획 시각화
        roadmap_data_list = []
        for year, semesters in ROADMAP_ACADEMIC.items():
            for semester, activities in semesters.items():
                # 직무별 심화 활동 추가 (고2 1, 2학기에만 적용)
                if year == '고2':
                    job_sim_activities = ROADMAP_JOB_SIMULATOR[selected_job].get('고2_심화', [])
                    activities = activities + job_sim_activities if job_sim_activities else activities
                
                # 고3의 최종 목표는 핵심으로 표시
                core_goal = ""
                if year == '고3':
                    core_goal = f"({ROADMAP_JOB_SIMULATOR.get(selected_job, {}).get('고3_목표', '최종 목표 설정 중')})"

                roadmap_data_list.append({
                    '학년/학기': f"{year} {semester}",
                    '학습 목표': ROADMAP_ACADEMIC[year]['목표'] if semester == '1학기' and year != '고3' else '', # 목표는 학년 시작 시점에 한 번만 표시
                    '주요 활동': "\n".join(activities) + core_goal
                })

        df_academic = pd.DataFrame(roadmap_data_list)
        
        # '학습 목표'를 병합하여 깔끔하게 보이도록 설정 (Streamlit 표에서는 직접 병합이 어려우므로 텍스트로 처리)
        def merge_goal(row):
            if row['학습 목표']:
                return f"**{row['학습 목표']}**\n{row['주요 활동']}"
            return row['주요 활동']
        
        df_academic['세부 계획'] = df_academic.apply(merge_goal, axis=1)
        df_academic = df_academic[['학년/학기', '세부 계획']]
        st.table(df_academic)

        st.markdown("---")

        # 3. 고3 이후 최종 진로 경로 (선택의 기회) - 이미지와 동일한 Expander 목록 유지
        st.markdown("### 고3 이후 최종 진로 경로 (선택의 기회)")
        
        # 아우스빌둥
        with st.expander("아우스빌둥 (일학습 병행) - 메인 취업처"):
            st.markdown("##### **[벤츠, BMW, 아우디 등 수입차 브랜드 취업]**")
            st.write(f"- **전형:** 5월 필기 -> 6월 면접 -> 9월부터 현장실습")
            st.write(f"- **혜택:** 대학 등록금 지원, 매월 **100만원 지원금** + 신입 정직원 급여 지급.")
        
        # 일반 취업/재직자 전형
        with st.expander("일반 취업 후 재직자 전형 (대학 진학 목표)"):
            st.markdown("##### **[우수 협력/선배 운영 업체 취업]**")
            st.write("- **경로:** 고3 10월 이후 3개월 현장실습 -> 1월 근로계약 체결 및 취업.")
